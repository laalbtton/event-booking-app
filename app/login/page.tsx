...
import { persistOAuthProfileFields, startOAuthSignIn, type OAuthProvider } from '@/lib/oauthClient'
...
function LoginContent() {
  ...
  async function handleOAuthLogin(provider: OAuthProvider) {
    ...
    try {
      ...
      const { native } = await startOAuthSignIn(provider)
      // Native: CapacitorProvider finishes the session after the custom-scheme
      // callback. Keep loading=true so the user stays on this screen until then.
      if (native) return
    } catch (error: any) {
      setError(error.message)
      setLoading(false)
      ...
      <Card className="max-w-md w-full shadow-2xl border-red-600/55 bg-zinc-900 text-stone-100">
        ...
        <CardContent className="space-y-6">
          ...
          <Button
            ...
            disabled={loading}
            className="w-full h-11 text-base font-medium bg-white text-black hover:bg-stone-200"
            type="button"
          >
            <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24" aria-hidden="true" fill="currentColor">
              <path d="M16.365 1.43c0 1.14-.415 2.2-1.207 3.03-.83.87-2.2 1.54-3.35 1.45-.15-1.1.4-2.27 1.19-3.08.85-.9 2.3-1.55 3.37-1.4zM20.75 17.14c-.57 1.32-.84 1.9-1.57 3.07-1.02 1.62-2.46 3.64-4.24 3.66-1.58.03-1.99-1.03-4.14-1.02-2.16.01-2.6 1.06-4.18 1.03-1.78-.03-3.14-1.84-4.16-3.46-2.86-4.52-3.16-9.83-1.4-12.64 1.25-1.99 3.23-3.16 5.1-3.16 1.9 0 3.1 1.07 4.68 1.07 1.54 0 2.48-1.08 4.69-1.08 1.67 0 3.44.91 4.68 2.48-4.11 2.25-3.44 8.12.54 9.05z"/>
            </svg>
            Continue with Apple
          </Button>
