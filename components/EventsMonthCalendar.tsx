...
export function EventsMonthCalendar({ events, emptyDayText = 'No events on this day.' }: Props) {
  ...
  return (
    <section className="space-y-4">
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-3 border-b border-border">
          <div className="flex items-center gap-1">
            ...
            <h2 className="text-sm sm:text-base font-semibold min-w-[9.5rem] text-center select-none">
        ...
        <div className="flex gap-4 px-3 py-2 border-b border-border bg-muted/20 text-xs text-muted-foreground">
        ...
        >
          <div className="grid grid-cols-7 border-b border-border bg-muted/10">
            {DAY_LABELS.map((label) => (
              <div key={label} className="py-1.5 text-center text-[11px] font-medium text-muted-foreground">
                {label}
              </div>
            ))}
          </div>
            ...
            {monthCells.map((cell, idx) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'min-h-[52px] sm:min-h-[88px] border-b border-r border-border p-1 text-left align-top',
                ...
                >
                  <span
                    className={cn(
                      ...
                      !isToday && cell.inMonth && isPastDay && 'text-muted-foreground',
                      !isToday && cell.inMonth && !isPastDay && 'font-medium'
                    )}
                  >
                    {cell.date.getDate()}
                  </span>
                  ...
                  <div className="mt-0.5 flex flex-wrap gap-0.5 sm:hidden">
                    {upcomingCount > 0 && <span className="h-1.5 w-1.5 rounded-full bg-primary" />}
                    {pastCount > 0 && <span className="h-1.5 w-1.5 rounded-full bg-muted-foreground/50" />}
                  </div>
      ...
      <div>
        <h3 className="text-sm font-semibold mb-3">{formatSelectedDate(selectedDate)}</h3>
                ...
                <li key={event.id}>
                  ...
                  >
                    <div className="w-16 shrink-0 pt-0.5">
                      <p className="text-xs font-semibold text-primary">{formatEventTime(event.startDate)}</p>
                      {isPast && (
                        <p className="text-[10px] uppercase tracking-wide text-muted-foreground mt-1">Past</p>
                      )}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="font-semibold leading-snug">{event.title}</p>
                      {event.subtitle && (
                        <p className="text-sm text-muted-foreground mt-0.5 truncate">{event.subtitle}</p>
                      )}
                      {event.badge && (
                        <p className="text-xs text-muted-foreground mt-1">{event.badge}</p>
                      )}
                    </div>
                  </Link>
                </li>
