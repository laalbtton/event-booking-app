    ...
    const timer = setTimeout(() => {
      void (async () => {
        try {
          ...
          const res = await fetch(`/api/profiles/search?q=${encodeURIComponent(term)}`, { headers })
      ...
      <div className="min-h-screen bg-background pb-24">
        <div className="max-w-2xl mx-auto px-4 py-6 sm:px-6 space-y-5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-2xl font-bold">Find performers</h1>
              <p className="text-sm text-muted-foreground">Search comics by name. Audience members are not listed.</p>
            </div>
          </div>
          ...
          {term.length < MIN_QUERY_LENGTH ? (
            <Card>
              <CardContent className="p-6 text-center space-y-2">
                ...
                <p className="font-medium">Search for a performer</p>
                <p className="text-sm text-muted-foreground">
                  Type at least {MIN_QUERY_LENGTH} characters of their name. Follow them to see
                  upcoming gigs in your feed
                  {!user ? ' — sign in to follow.' : '.'}
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-6 text-center space-y-2">
                <p className="font-medium">No performers found</p>
                <p className="text-sm text-muted-foreground">
                  Nobody matches &ldquo;{term}&rdquo;. Try a different spelling.
                </p>
              </CardContent>
            </Card>
          ) : (
