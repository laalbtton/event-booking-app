...
export function PublicEventCTA({
  ...
  // Attend form state (open mic magic link)
  ...
  const canCheckoutTickets = isBookedShow && !!ticketsEnabled && !ticketUrl && hasPricedTickets
  const hasExternalTickets = isBookedShow && !!ticketsEnabled && !!ticketUrl
  ...
  return (
    <div className="rounded-xl border border-zinc-700 bg-zinc-900 shadow-sm p-5 space-y-4">
      {isBookedShow ? (
        <>
          {/* Ticketed booked show — guest Stripe checkout is the primary action (no login required) */}
          {canCheckoutTickets ? (
        ...
        <>
          ...
          {ticketsEnabled && !isFree && signupNudge}
        </>
      )}
